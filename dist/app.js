...
<body>
...
</html>`),t.document.close()}async function J(n,e,t,r){let i=mr(n.id),s;try{s=await fr(i)}catch{s='<p style="color:#f87171">QR generation failed</p>'}let o=U(`
  ...
  `);document.getElementById("qr-print-btn")?.addEventListener("click",()=>pr(s,n)),document.getElementById("qr-done-btn")?.addEventListener("click",()=>{$(o),r&&r()})}var ye=class{constructor(e,t,r,i){this.container=e;this.app=t;this.freezerId=r;this.shelfNumber=i;this.items=[];this.freezer=null;this.allFreezersList=[];this.selectedId=null}async render(){let[e,t]=await Promise.all([this.app.storage.getItems(),this.app.storage.getSettings()]);this.allFreezersList=t.freezers,this.freezer=t.freezers.find(s=>s.id===this.freezerId)??null,this.items=e.filter(s=>s.freezerId===this.freezerId&&s.shelf===this.shelfNumber).sort((s,o)=>new Date(s.storedAt).getTime()-new Date(o.storedAt).getTime());let i=`${this.freezer?.name??"Freezer"} \u2014 Shelf ${this.shelfNumber}`;this.container.innerHTML=`
